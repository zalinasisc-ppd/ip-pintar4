...
EXPECTED_COLUMNS = {
    ...
    "%LSemasa": ["%LSemasa", "LSemasa"],
    "%LLepas": ["%LLepas", "LLepas"],
}
...
def compute_ipmp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["Mata Pelajaran", "Bil. Ambil", "%LSemasa", "%LLepas"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"Kolum '{c}' tiada dalam data.")
    # Coerce numerics
    for c in ["Bil. Daftar", "Bil. Ambil", "%LSemasa", "%LLepas"]:
        if c in df.columns:
...
st.sidebar.markdown("---")
st.sidebar.caption("Kolum diperlukan: Mata Pelajaran, Bil. Ambil, %LSemasa, %LLepas. (Bil. Daftar opsyenal).")
...
# Main UI
# -----------------------------
st.title("📊 SiPP — PERBANDINGAN PERATUS LULUS PPT SEMASA VS TAHUN LEPAS")
...
st.subheader("Perbandingan Peratus (LLepas vs LSemasa)")
long_df = df_result.melt(id_vars=["Mata Pelajaran"], value_vars=["%LLepas", "%LSemasa"],
                         var_name="Jenis", value_name="Peratus")
fig2 = px.line(long_df, x="Mata Pelajaran", y="Peratus", color="Jenis",
               markers=True, title="Peratus Lepas vs Semasa Mengikut Subjek")
fig2.update_layout(xaxis_title="", yaxis_title="Peratus", margin=dict(t=60, b=20))
